 ...
 */
bool is_inside(SDL_Rect bounds, SDL_Rect check)
{
    // Check that the x and y are within and the bounds.
    // Top left.
    if (check.x < bounds.x || check.y < bounds.y ||
        // Bottom Right (x axis)
        check.x + check.w > bounds.x + bounds.w ||
        // Bottom Right (y axis)
        check.y + check.h > bounds.y + bounds.h)
        return false;
...
Quadrent get_dir(SDL_Point centre, SDL_Point point)
{
    if (point.x == centre.x && point.y == centre.y)
        return TOPLEFT; // DIRECTIONS; Only one per node at the moment.
